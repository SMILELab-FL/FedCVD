...
from algorithm.echo.fedconsist import FedConsistServerHandler, FedConsistSerialClientTrainer
...
parser.add_argument("--unlabeled_lr", type=float, default=0.001)
...
if __name__ == "__main__":
    ...
    sample_ratio = 1
    unlabeled_lr = args.unlabeled_lr
    tau = args.tau
    ...
    trainer = FedConsistSerialClientTrainer(
        tau=tau,
        model=model,
        num_clients=num_clients,
    ...
    handler = FedConsistServerHandler(
        num_classes=args.n_classes,
        model=model,
        test_loaders=test_loaders,
        criterion=criterion,
