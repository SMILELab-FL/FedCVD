...
class FedConsistServerHandler(SyncServerHandler):
    ...
    def __init__(
        ...
        super(FedConsistServerHandler, self).__init__(model, test_loaders, criterion, output_path, evaluator, communication_round, num_clients, sample_ratio, device, logger)
...
class FedConsistSerialClientTrainer(SGDSerialClientTrainer):
    def __init__(
            self,
            tau,
            model,
        ...
        super(FedConsistSerialClientTrainer, self).__init__(model, num_clients, train_loaders, test_loaders, device, logger, personal)
        self._model = deepcopy(model).to(self._device)
        self.tau = tau
        self.labeled_lr = labeled_lr
    ...
    def train(self, epoch, idx):
        ...
        if idx == 0:
            ...
            for data, label, mask in self.train_loaders[idx]:
                ...
                data, label = data.to(self._device), label.to(self._device)
                pred_score = self._model(data + torch.randn(data.size()).to(self._device) * 0.1)
                with torch.no_grad():
                    pred_index = self._model(data)
                    for i in range(pred_score.shape[1]):
                        label[:] = torch.where((pred_index[:, i] > self.tau) & (pred_score[:, i] > 0.5), i, label)
                loss = self.criterion(pred_score, label)
