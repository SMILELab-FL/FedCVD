...
from algorithm.ecg.fedavg import FedAvgServerHandler, FedAvgSerialClientTrainer, FedAvgNonIIDSerialClientTrainer, FedAvgNonIIDServerHandler
...
class DittoServerHandler(FedAvgServerHandler):
...
class DittoSerialClientTrainer(FedAvgSerialClientTrainer):
    def __init__(
            self,
            mu,
            model,
        ...
        super(DittoSerialClientTrainer, self).__init__(
            model=model,
    ...
    def local_process(self, payload, id_list):
        ...
        for idx in id_list:
            ...
            for epoch in range(self.max_epoch):
                pack = self.train_global_model(epoch, idx)
            self.cache.append(pack)
            frz_model = deepcopy(self._model)
            SerializationTool.deserialize_model(frz_model, model_parameters)
            self.set_model(self.local_models[idx])
            for epoch in range(self.max_epoch):
                self.local_models[idx] = self.train_local_model(epoch, frz_model, idx)
    ...
    def train_local_model(self, epoch, frz_model, idx):
        self._model.train()
